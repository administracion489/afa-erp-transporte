...
import { buildHeaderPDFHtml, buildFooterPDFHtml, sharedCSS, LOGO_DEFAULT } from "./pdf-chrome";
...
function buildSeccion(titulo: string, sentido: string, color: string, filas: FilaFicha[], mapa: string, metrica: MetricaSentido | null): string {
  if (!filas.length) {
  ...
  const img = mapa
    ? `<div class="ficha-mapa"><img src="${mapa}" onerror="this.closest('.ficha-mapa').style.display='none'"/><p>Recorrido referencial por vía — ${esc(sentido)}</p></div>`
    : "";
...
export function buildFichaRutaHtml(d: FichaRutaDatos): string {
  ...
  const css = sharedCSS(`
    ...
    .ficha-kpis{display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin:0}
    ...
    .ficha-mapa{margin-top:8px;text-align:center;page-break-inside:avoid}
    .ficha-mapa img{width:100%;max-width:100%;border:1px solid #d7dde5;border-radius:5px}
    .ficha-mapa p{margin:3px 0 0;font-size:8px;color:#94a3b8}
    .ficha-qr{border:1px solid #d7dde5;border-radius:5px;padding:8px;text-align:center;width:134px;flex-shrink:0}
    .ficha-qr img{width:106px;height:106px;display:block;margin:0 auto}
    .ficha-qr p{margin:4px 0 0;font-size:7.5px;color:#64748b;line-height:1.3}
...
<div style="display:flex;gap:10px;align-items:stretch;margin-bottom:4px;">
  <div style="flex:1;"><div class="ficha-kpis">${kpis}</div></div>
  ${d.qr ? `<div class="ficha-qr"><img src="${d.qr}"/><p>Escanea para abrir el recorrido de entrada en Google Maps</p></div>` : ""}
</div>
...
${buildSeccion("Puntos de entrada (ida)", "ida", C_IDA, filasIda, d.mapaIda, d.metrica?.ida || null)}
${buildSeccion("Puntos de retorno (salida)", "retorno", C_RET, filasRet, d.mapaRet, d.metrica?.retorno || null)}
<div class="ficha-sec">
  <div class="ficha-sec-h" style="border-color:${CP_FICHA};"><span style="color:${CP_FICHA};">Unidad asignada</span></div>
  <div class="box"><div class="box-row"><b>${esc(d.unidadTexto)}</b></div>${d.unidadDetalle ? `<div class="box-row" style="color:#475569;">${esc(d.unidadDetalle)}</div>` : ""}</div>
</div>
<div class="ficha-sec">
...
<div class="ficha-firmas">
  <div class="ficha-firma"><div class="linea"></div><p class="rol">${esc(d.empresa.nombre)}</p><p>${esc(d.repr)}</p><p>Coordinación de operaciones</p></div>
