...
export const MODULOS_FLOTA: AyudaModulo[] = [
    ...
    faqs: [
      {
        ...
        respuesta:
          "No lo decides tú: **lo calcula el sistema mirando los documentos obligatorios de esa placa**.\n\nSi alguno de los papeles obligatorios que tienes cargados está vencido, la unidad pasa a **No apto** y aparece con el punto rojo. Si ninguno está vencido, queda **Apto**.\n\nDos cosas que conviene tener claras:\n\n• El ERP marca **seis** documentos como obligatorios por placa: SOAT, CAT, Revisión Técnica (CITV), Tarjeta de Propiedad, Tarjeta Única de Circulación (TUC) y Tarjeta de Circulación. Para decidir Apto/No apto se miran **cinco**: el CAT queda fuera a propósito, porque es la alternativa al SOAT en provincias y no todas las unidades lo llevan. Los seis sí cuentan para la barra de Completitud, y por eso esa barra llega a **6/6**.\n\n• **La Habilitación Vehicular (MTC/ATU) ya NO se le pide a la placa.** Es UNA sola y es de la empresa —la madre—, y de ella salen las TUC de cada vehículo —las hijas—. Si un bus tiene TUC, la empresa está habilitada por lógica; pedirla también por unidad era un pendiente que solo se podía cerrar colgándole al bus una copia del papel de la empresa.\n\n• **La Tarjeta de Propiedad no vence nunca.** La emite SUNARP y no trae fecha de vencimiento: no le pongas una. En el ERP sale como **No vence** y solo se le pregunta una cosa — si está cargada o no. Se reemplaza cuando cambian el titular o las características del vehículo, pero eso no es una renovación con fecha.\n\n• **El semáforo solo ve lo que está cargado.** Un documento que nunca registraste no cuenta como vencido: cuenta como inexistente, y la unidad puede salir “Apto” con el papel faltando de verdad. Por eso la pantalla Documentos Vehiculares tiene la barrita de **Completitud** (por ejemplo 5/7): esa es la que te dice qué falta.\n\nOjo también con la columna **Disponibilidad** (Disponible / Ocupado / En mantenimiento / Inactivo): esa sí la pones tú a mano y responde otra pregunta — no si la unidad *puede* salir, sino si *está libre*.",
      },
      {
        pregunta: "¿Para qué sirve el semáforo de vencimientos y qué pasa de verdad si se me vence un SOAT o una CITV?",
        respuesta:
          "El semáforo tiene tres colores y una sola intención: que renueves antes, no después.\n\n• **Vigente** (verde): faltan más de 30 días.\n• **Por vencer** (amarillo): quedan 30 días o menos. Es tu ventana para trámite.\n• **Vencido** (rojo): ya se pasó la fecha.\n\nQué pasa si se vence, en concreto:\n\n• **SOAT vencido** — circular sin SOAT vigente es infracción grave. Se sanciona con multa y **retención o internamiento de la unidad**, y si hay un accidente el seguro no responde: los daños a terceros los pagas tú, de tu bolsillo.\n• **Revisión Técnica (CITV) vencida** — multa e inmovilización. En un servicio de transporte de personal, además, es lo primero que te pide el cliente en una fiscalización.\n• **Tarjeta Única de Circulación (TUC) o Habilitación Vehicular (MTC/ATU) vencidas** — no es una papeleta de tránsito, es habilitación: sin eso la unidad no está autorizada a prestar el servicio, y el problema pasa de ser del chofer a ser de la empresa. (Son los dos documentos que el ERP llamaba antes “Habilitación SUTRAN” y “Permiso Operación MTC”; lo que cambió es el nombre, no el papel.)\n\nEl costo real de un vencimiento casi nunca es la multa: es el servicio que no puedes cubrir esa mañana y el cliente que se entera.\n\n**Los montos y las consecuencias exactas dependen del reglamento vigente y de qué autoridad te intervenga.** Úsalo como semáforo de gestión, y confirma el detalle sancionador con tu asesor.",
      },
      {
        pregunta: "¿Para qué es la “Capacidad de tanque” y por qué a veces me avisa en ámbar bajo la categoría de costeo?",
        respuesta:
          "Son dos cosas distintas que se apoyan la una en la otra.\n\n**La capacidad de tanque** dice cuánto le entra a ESA unidad, por tipo de combustible. Sirve para que el ERP detecte una carga imposible: si el voucher dice 95 galones y al bus le entran 80, algo se leyó mal o se cargó a la placa equivocada. Déjalo vacío y se usa un estimado por categoría, que es grueso — para un kit GLP es especialmente malo, porque un kit convertido lleva 20-30 galones y el estimado de un bus asume 80.\n\n**Una unidad puede declarar DOS tanques y eso no es un error**: las unidades bicombustible (GLP + gasolina, por ejemplo) cargan las dos cosas. Declarar los dos es lo correcto.\n\n**Lo que NO es este campo**: no es “el combustible de este vehículo”. El tipo de una recarga **sale siempre del producto impreso en el voucher**, nunca de la ficha del bus — si el ERP dedujera el combustible del vehículo, el día que esa unidad cargue lo otro lo registraría mal, con el precio de referencia equivocado.\n\n**El aviso ámbar** sale cuando la categoría de costeo que elegiste costea en un combustible que esta unidad no declara poder cargar. Importa porque el costo por kilómetro es **precio ÷ rendimiento**: con el combustible equivocado el renglón no se desvía un 10 %, se desvía en la diferencia entre dos precios, y el diésel y el GLP se diferencian como 3 a 1. Se corrige en el desplegable Combustible de Configuración → Costos, o cambiando la unidad de categoría.\n\nSi no declaraste ningún tanque, el ERP **no te dice que discrepe**: no tiene con qué compararlo, y prefiere callarse antes que afirmar algo que no sabe.",
      },
      {
    ...
    comoHacer: [
      {
        ...
        pasos: [
          ...
          "Elige el nivel de servicio (⭐ Full Equipo o 📦 Estándar): el texto de la descripción se rellena solo y es el que verá el cliente en la cotización.",
          ...
          "Guarda. Después, con la unidad ya creada, pulsa “+ Doc” en su fila y carga uno por uno los documentos obligatorios: SOAT, CAT, CITV, Tarjeta de Propiedad, TUC y Tarjeta de Circulación, cada uno con su fecha de vencimiento — salvo la Tarjeta de Propiedad, que no tiene: el ERP ni te la pide.",
        ],
        advertencia:
          "Un documento sin fecha de vencimiento sale como “Sin fecha” y NO dispara alerta. Si no tienes la fecha a la mano, es mejor no dar por cerrado el registro: revisa la barra de Completitud en Documentos Vehiculares hasta llegar a 6/6. La excepción es la Tarjeta de Propiedad: sale como “No vence”, es correcto y no hay nada que completar.",
      },
    ],
    relacionadas: [
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    faqs: [
      ...
      {
        pregunta: "¿Dónde pongo lo que costó un mantenimiento, y cómo subo la factura del taller?",
        respuesta:
          "Dentro de la **Orden de Trabajo**, y de tres formas que no se pisan:\n\n• **Con el desglose** (“+ Agregar línea”) — cuando el repuesto lo compraste en un sitio y la mano de obra la pagaste en otro, o cuando parte del trabajo lo hizo tu propio mecánico. Es el camino completo: cada línea lleva su proveedor y su factura. Tiene su propia pregunta más abajo.\n• **Por ítem** — al lado de cada repuesto del checklist hay ahora su costo (el papel impreso ya traía ahí un “Costo: S/ ______” para llenar a mano). Si pones el costo de algún ítem, el **total de la orden se calcula solo** con la suma y el campo del total se bloquea: así el encabezado nunca dice un número distinto del que suma su propio detalle.\n• **Un total global** — si la factura del taller viene con un solo importe, déjalo en “Costo total S/” y no toques los ítems. Repartir un importe global entre catorce líneas sería inventarse el reparto.\n\n**Lo que se arregló y conviene saber:** antes el costo solo viajaba al libro de mantenimiento **en el instante de cerrar la OT**. Una orden automática nace en S/ 0.00, se cerraba en S/ 0.00 y esa fila se quedaba en cero para siempre — ponerle el costo después no movía ni el gasto, ni el margen del servicio, ni el costo por kilómetro de su categoría. Ahora el número baja al libro **cada vez que lo cambias**, esté la orden abierta o ya cerrada.\n\n**La factura**: botón **🧾 Registrar factura del taller** dentro de la orden. Pides RUC, serie, número, fecha e importe, y adjuntas el PDF o la foto. Eso crea la **Cuenta por Pagar** del taller en Tesorería: desde ahí se aprueba, entra a un lote de pago, se concilia contra el banco y sustenta el crédito fiscal del IGV. Si esa misma factura ya la había registrado Contabilidad, **no se duplica**: la orden se enlaza a la que ya existe y el ERP te lo dice.\n\n**Ojo con una cosa que parece un error y no lo es:** el importe de la factura suele ser mayor que el costo de la orden, justo en un 18 %. Es el IGV. El costo del taller va **sin IGV** a propósito, porque el IGV lo recuperas como crédito fiscal; si lo metieras, el costo por kilómetro de toda esa categoría subiría 18 % y encarecería cada cotización nueva. Cuando la diferencia es exactamente esa, el ERP te lo explica en azul y no te pide corregir nada. Cuando es otra, te avisa en ámbar para que mires cuál de los dos números está mal.\n\nY no: **la factura no se sube a “Gastos”**. El gasto de mantenimiento ya se cuenta desde la propia orden; meterlo además en Gastos contaría el mismo sol dos veces en Finanzas.",
      },
      {
        pregunta: "Compré el repuesto en un sitio y pagué la mano de obra en otro. ¿Y cuando lo haga mi propio mecánico?",
        respuesta:
          "Para eso está el **desglose** de la orden: dentro de la OT, debajo del costo, un botón **“+ Agregar línea”**. Cada línea lleva cuatro cosas que un costo plano no puede contestar:\n\n• **Qué es** — repuesto/material, mano de obra o servicio del taller.\n• **De dónde sale** — *Comprado* o *Propio / de casa*. Esto es lo importante, sigue leyendo.\n• **A quién** — el proveedor, que puede ser distinto en cada línea.\n• **Con qué papel** — cada línea tiene su propio botón 🧾 para registrar **su** factura.\n\nAsí una sola orden lleva el kit de embrague de la tienda de repuestos, la mano de obra del taller de la esquina y —el día que tengas mecánico— sus horas, sin inventar tres órdenes de trabajo distintas.\n\n**“De casa” no es una etiqueta: decide si ese dinero sale de la caja.** Lo *comprado* entra como gasto del vehículo en Finanzas y admite factura. Lo *propio* **no**: la hora de tu mecánico ya la pagaste en la planilla, y el repuesto que sacas de tu almacén ya lo pagaste cuando lo compraste. Volver a contarlo como gasto sería contar el mismo sol dos veces.\n\n**Pero tampoco es gratis, y esto es lo que más cuenta.** Ese costo sí es de esa unidad, así que sí entra al **costo por kilómetro** de su categoría. Si no entrara, el día que contrates un mecánico el ERP vería que el taller “cuesta menos” —cuando lo único que pasó es que el costo se mudó a la planilla— y empezaría a ofertar esa categoría más barata sin que nadie lo decidiera. Por eso la orden te enseña **dos números con etiquetas distintas**: *salió de caja* y *de casa*.\n\n**Cuánto vale una hora de tu mecánico** lo calcula el ERP, no lo tecleas en cada orden. En la línea eliges **quién hizo el trabajo** y pones las horas; el monto sale solo. Si tienes **dos o más**, cada uno se valoriza con lo que de verdad cuesta su hora: para eso ponle su sueldo básico en **Personal Adm.** Quien no tenga sueldo configurado no aparece en ese selector — eso es lo que falta. Y si la línea no dice quién trabajó, se usa la tarifa de respaldo que configuras en **Próximos → Mano de obra del taller**. El monto sale por uno de dos caminos:\n\n1. **Su costo empresa real** — si esa persona tiene sueldo básico en su ficha, el ERP calcula lo que de verdad te cuesta al mes (sueldo + gratificaciones + CTS + EsSalud + SCTR, según tu régimen) y lo divide entre sus horas de taller. Es el número exacto, y es el que manda.\n2. **Una tarifa por hora** que tecleas una vez, para cuando el mecánico aún no está en planilla o prefieres no poner su sueldo ahí.\n\nSi no hay ninguno de los dos, el campo se queda bloqueado y te dice qué llenar: **no se inventa un S/hora**, porque ese número multiplica las horas de cada orden y termina moviendo el precio de venta de toda una categoría.\n\n**Ojo con quien va por recibo por honorarios:** a esa persona se le paga POR el trabajo, así que su línea va como **Comprado** con el importe de su recibo, no como hora propia. Si la imputaras, esa plata se quedaría fuera de tus gastos — y sí salió de la caja. El ERP te lo dice al elegirla.\n\n**Una advertencia honesta:** lo que imputas son las horas que cargas a las órdenes. El resto del mes de tu mecánico (esperas, traslados, taller barrido) no lo absorbe nadie, así que ese costo por kilómetro es un **piso**, no la cuenta completa del taller.\n\nY si una orden acaba con **dos facturas**, el historial no guarda ninguna de las dos como “la factura de la orden”: elegir una haría creer que la otra no existe. Las dos están igual en Cuentas por Pagar, cada una con su proveedor, y se aprueban y concilian por separado.",
      },
      {
      ...
      {
        pregunta: "La IA leyó el odómetro con un dígito de más. ¿Qué hace el ERP y qué tengo que hacer yo?",
        respuesta:
          "Es el error más frecuente de esta lectura: el tablero marca **23,980** y la IA devuelve **239,980**. Diez veces el número real, y en una foto de tablero las dos cifras se parecen.\n\n**Lo que hace el ERP, por orden:**\n\n• **Se lo dice al modelo antes de que lea.** El ERP sabe cuántos dígitos tiene el odómetro de cada unidad (sale de su kilometraje vigente) y ahora se lo pasa **siempre** — antes solo lo hacía en las unidades que tenían escrita a mano una “guía de odómetro”, es decir en casi ninguna. Con “el odómetro de esta unidad es un número de 5 dígitos” delante, una respuesta de 6 cifras se cae sola.\n\n• **Nunca te pre-llena el número inflado.** Si el que devuelve es imposible para la unidad, el campo se queda vacío y el aviso te dice exactamente qué pasó: *“le sobra un dígito: leyó 6 y esta unidad tiene 5”*, con el rango en el que tiene que estar el número. Antes decía solo “no cuadra”.\n\n• **Si igual entra, no toca el kilometraje vigente.** Una lectura así queda **sospechosa** en la bandeja “Lecturas por revisar” de esta pestaña, con su foto. El km de la unidad no se mueve hasta que una persona la acepte.\n\n• **Si el dígito que sobra está DUPLICADO, te propone el número.** Midiendo los casos reales de tu flota se vio que la IA no inventa una cifra cualquiera: **repite una que ya estaba** (23,980 lo devolvió como 239,**9**80; 23,379 como 233,**3**79; 560,473 como 5,6**0**0,473). Cuando quitar esa repetición da **un solo** valor posible para la unidad, el ERP lo pone en el formulario y te enseña los dos números: *“la IA leyó 239,980, se propone 23,980”*. **Compruébalo contra la foto igual** — ese número lo dedujo el sistema, no lo leyó nadie. Y cuando salen dos valores posibles, o ninguno, no elige: te lo deja en blanco.\n\n**Lo que tienes que hacer tú: mirar la foto y confirmar.** El ERP nunca recorta «por donde cuadre»: de 239,980, borrando cualquier dígito, también caben 23,990 y 23,998, y ninguno es lo que marca el tablero. Un km inventado se propaga al vencimiento del mantenimiento y al rendimiento km/galón de esa unidad.\n\n**Y usa “✕ Rechazar”, no lo dejes ahí.** Ese botón abre la corrección: eliges **“La IA leyó mal un dígito”** y escribes el km correcto. Eso hace dos cosas — deja la lectura buena en su sitio y **le enseña el caso a la IA**, que lo recibe como ejemplo la próxima vez que lea un tablero de esa placa.",
      },
    ],
    comoHacer: [
      {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    faqs: [
      ...
      {
        pregunta: "¿Por qué el número grande de arriba es el costo por kilómetro?",
        respuesta:
          "Porque es **lo único que se puede comparar entre las cuatro familias de combustible de tu flota**. El GNV se vende por metro cúbico y el diésel, la gasolina y el GLP por galón: un «km/m³» y un «km/gal» no son el mismo número, no se pueden sumar y no se pueden promediar. Soles por kilómetro sí.\n\nSe calcula **soles gastados ÷ kilómetros recorridos**, sobre los tramos cuyo kilometraje es de fiar. Y ojo con esto, porque es distinto del rendimiento: un tramo puede NO poder decir cuánto rinde la unidad —porque es bicombustible, porque falta una carga por registrar, porque hubo una carga sin odómetro en medio— y aun así **saber perfectamente cuántos kilómetros recorrió**. Esos kilómetros cuentan para el costo por km. Antes no contaban, y el costo salía calculado sobre una minoría de las cargas y más alto de lo real.\n\n**Al lado va la cobertura**, y no es decoración: si dice «cubre el 45 % del gasto», ese costo por km describe menos de la mitad del periodo. Lo que falta son cargas sin kilometraje — ponérselo lo sube.\n\nEl **rendimiento por combustible sigue estando**, unidad por unidad, en la tabla de abajo. Es el dato secundario: sirve para comparar una unidad contra sí misma, no contra otra que carga otra cosa.",
      },
      {
        pregunta: "¿Qué es la columna «Últimas 5» y en qué se diferencia del rendimiento de la fila?",
        respuesta:
          "Es el rendimiento de **los últimos cinco tramos juntos** (los kilómetros de todos ellos divididos entre todo el combustible de todos ellos), con el **porcentaje de diferencia contra la mediana histórica** de esa unidad.\n\n**Para qué sirve, que es lo que no se ve mirando fila por fila**: el número de una sola carga se mueve por cualquier cosa —un tanque que cortó antes, una semana de cerro, un tramo de carretera—, así que una caída real del 10 % no se distingue del ruido. Y la mediana, que es con lo que el ERP juzga cada carga, se mueve despacio a propósito: son meses de historia, así que una degradación reciente queda **diluida dentro de ella justo mientras está ocurriendo**.\n\nLa ventana de cinco separa las dos cosas. Si ves **−15 %** ahí tres o cuatro cargas seguidas, eso es una tendencia, no una fila rara — y se ve en unas dos semanas en vez de esperar al cierre de mes.\n\n**No lleva color de alarma**, y es a propósito: en GLP y GNV hay un 3-5 % de variación que no se puede quitar (la válvula corta al 80 % por norma y el gas cambia de volumen con la temperatura), así que un mismo umbral para las cuatro familias sería ciego en diésel y ruidoso en gas. El ERP publica el número; el juicio es tuyo.\n\nAl principio de una unidad dice «2 de 5»: la ventana no se inventa los tramos que todavía no existen.",
      },
      {
        pregunta: "¿Qué es la casilla «El tanque quedó lleno» y qué pasa si la desmarco?",
        respuesta:
          "Es lo que hace **válida** la medición de rendimiento, y viene marcada por defecto porque la política de la empresa es cargar a tope.\n\n**Por qué funciona.** Si el tanque queda lleno al empezar y lleno al terminar, lo que despachó la bomba al final es **exactamente** lo que se consumió en el camino — el nivel que quedaba antes de cargar, que nadie mide, se cancela solo en la cuenta. Por eso no importa cada cuántos días se recargue.\n\n**Si desmarcas la casilla** (no había crédito, el grifo no tenía stock), esa carga **no cierra una medición**: su rendimiento sale como «tanque parcial» en vez de un número. Y aquí está lo importante — **su combustible NO se pierde**: se suma al del próximo tanque lleno y ahí se mide, con los kilómetros de los dos tramos juntos. El resultado es correcto igual.\n\n**Por qué importa marcarla bien.** Una carga parcial que se declara llena hace que el rendimiento salga **inflado**, y eso el ERP lo lee como «falta registrar una carga» — es decir, te manda a buscar un repostaje que nunca existió. Con la casilla puesta, el sistema sabe cuál de las dos cosas es.\n\n**Y si nadie la toca, no pasa nada.** Una carga que no declara si quedó llena —todo el histórico, y las que registra sola el **Radar IA** desde una foto de WhatsApp— se sigue midiendo igual que siempre, apoyada en la política de la empresa. La casilla solo cambia algo cuando alguien la DESMARCA.\n\n**El Radar también la tiene, y ahí la propone la foto.** En **Radar IA → Combustible**, al revisar una recarga, sale la misma casilla. Viene marcada salvo que la IA haya visto en la foto del tablero que la aguja **no** llegó al tope: en ese caso la recarga **no se registra sola**, se queda esperando que la confirmes. Debajo de la casilla el ERP te enseña lo que sabe: si el importe es un número redondo (S/ 100.00, S/ 150.00) suele significar que se cargó **hasta un monto** —el caso de «no había saldo de crédito»— y si la cantidad es casi el tanque entero, lo más probable es que sí quedara lleno. **Esos indicios no marcan ni desmarcan nada**: solo te dicen dónde mirar. Un «parcial» puesto por una máquina dejaría de medir ese tramo en silencio, y eso no se nota en ninguna pantalla.",
      },
      {
        pregunta: "¿Por qué me pide un motivo cuando pongo el kilometraje?",
        respuesta:
          "Porque el salto desde la carga anterior **no cabe en un tanque de esa unidad**. El ERP toma la capacidad del tanque que declaraste en la ficha del vehículo y el rendimiento que esa unidad viene haciendo, multiplica, y compara.\n\nCasi siempre significa **una carga que se hizo y nadie registró** — el bus no recorrió 900 kilómetros con un tanque que da 700. La otra posibilidad es que el kilometraje esté mal tecleado.\n\n**No te bloquea el registro: te pide que escribas qué pasó.** La carga se guarda igual con el motivo puesto. Se hace así porque una carga que no se llega a guardar desaparece de la pantalla donde se corrige, y un salto sin explicar envenena después el rendimiento de la unidad y la fecha de su próximo mantenimiento.\n\n**Cuándo NO aparece**: si esa unidad todavía no tiene suficientes tramos medidos para saber cuánto rinde, o si no le declaraste la capacidad de tanque. Sin esos dos datos el ERP no tiene con qué compararlo y prefiere callarse antes que inventar un techo.",
      },
      {
        pregunta: "Puse el precio y me salió un aviso ámbar debajo. ¿Qué está mirando?",
        respuesta:
          "Está mirando si ese número **puede ser un precio de esa unidad de medida**, que es una pregunta distinta de si está caro.\n\nCada combustible se vende en lo suyo: diésel, GLP y gasolina por **galón**, el GNV por **m³**, la urea por **litro**. Como todos se teclean en el mismo campo, el error fácil es meter el precio de una unidad en una fila que dice otra — y cuando eso pasa, **todo lo que viene después miente**: el rendimiento de esa unidad, el costo por kilómetro y el precio de referencia que alimenta el cotizador.\n\nLo útil del aviso es que, cuando puede, **te dice cuál sería la unidad correcta**: *«S/ 6.90 está fuera de lo que cuesta un galón de diésel, pero encaja como precio por litro»*. Eso es exacto, no una sospecha — un diésel a S/ 26 el galón son S/ 6.87 el litro, y la diferencia entre los dos es justo 3.785.\n\n**No te bloquea.** Puede que ese día el precio fuera raro de verdad. Guarda si estás seguro; el aviso está para que mires antes, no para discutir contigo.\n\n**No es lo mismo que «precio fuera de rango» del Radar.** Aquel compara contra el precio de mercado que tienes configurado en Costos y se mueve todos los meses; éste compara contra lo que físicamente puede costar esa unidad, y no se mueve nunca.",
      },
      {
        pregunta: "¿Por qué el total de galones no suma todo lo cargado del mes?",
        respuesta:
          "Porque **un metro cúbico y un galón no son la misma cosa y no se pueden sumar**, y tu flota usa los dos: el GNV se despacha en m³ y el diésel, el GLP y la gasolina en galones.\n\nUn solo número de «galones» que sumara las cuatro familias no sería un total de nada — sería una cifra sin unidad, y encima rotulada en una que no le corresponde a la mayor parte de la flota. Por eso cada tarjeta de arriba lleva **su** combustible con **su** unidad, y donde hace falta un total mezclado verás algo como *«398.6 gal · 1,240.0 m³»*, cada uno por su lado.\n\n**Lo único que sí se suma entre combustibles es el dinero**, y de ahí sale la métrica principal del módulo: el **costo por kilómetro**. Ese sí compara una unidad de GNV contra una de diésel sin trampa, y es el que tienes arriba del todo.\n\nLo mismo vale en el ranking de ELIA y en la analítica del vehículo: donde antes salía un «gal» fijo, ahora sale la unidad de cada familia.",
      },
      {
        pregunta: "¿Qué es el “rendimiento” y cuándo el ERP me marca una anomalía?",
        respuesta:
          "El **rendimiento** son los kilómetros que la unidad hizo por cada galón (o por m³ si es GNV). Se calcula solo: kilómetros entre esta carga y la anterior de esa misma unidad y ese mismo combustible, divididos entre lo que cargaste. Es el dato **secundario** del módulo: sirve para comparar una unidad contra sí misma. Para comparar unidades entre sí está el costo por kilómetro de arriba, porque un km/gal y un km/m³ no se pueden poner en la misma columna.\n\nLa referencia de cada unidad es su **mediana**, no su promedio: un tanque llenado a medias mueve el promedio y no mueve la mediana.\n\nEl ERP levanta la bandera 🚨 en tres casos:\n\n• **Carga excesiva** — cargaste más de lo que cabe en el tanque, con un 10 % de tolerancia. Si la CWQ-400 tiene tanque de 100 galones y aparece una carga de 118, algo no cuadra: o el tanque está mal configurado en la ficha del vehículo, o el galonaje del ticket está mal, o se cargó en dos unidades con un solo comprobante.\n\n• **Consumió de más** (rojo) — la unidad rindió 30 % o menos que su propia mediana. Se compara contra ella misma, nunca contra otras: un bus viejo con 8 km/gal no salta; ese mismo bus rindiendo 5 km/gal, sí.\n\n• **Falta registrar una carga** (ámbar) — rindió MÁS de lo que es físicamente posible. Suena a buena noticia y es lo contrario: significa que el combustible de ese tramo se compró y se consumió pero **no está en el libro**, así que el costo de esa unidad sale más bajo de lo real y el margen de sus servicios, más alto. Antes este caso salía en verde con ✓, y de paso subía el promedio de la unidad hasta marcar en rojo cargas que estaban perfectas.\n\n**Cuando la columna no muestra un número, dice por qué**, porque cada motivo se arregla en otro lado: *1ª carga* (todavía no hay tramo), *sin km* / *sin km antes* (a esa carga le falta el kilometraje — edítala y ponlo), *tramo incompleto* (hay cargas sin kilometraje en medio, así que su combustible no está en la cuenta), *km no avanza* (uno de los dos odómetros está mal tecleado), *tanque parcial* (alguien marcó que esa carga no llenó el tanque, así que no cierra una medición — su combustible se cuenta en el tramo siguiente, no se pierde), *bicombustible* (la unidad usa DOS combustibles y en ese tramo repostó del otro, así que parte de esos kilómetros se hicieron con algo que no está en esta cuenta — **no es una carga que falte registrar, y no hay nada que arreglar**: un km/gal por combustible solo se puede medir en tramos donde la unidad usó uno solo), *implausible* (el número sale absurdo: o faltan cargas de ese periodo, o el kilometraje está mal — el detalle dice cuál de los dos). Despliega la fila para ver la explicación completa.\n\n**Una anomalía no es una acusación.** Las causas honestas son la mayoría: tráfico, ruta con más pendiente, no se llenó el tanque completo, o el kilometraje se anotó mal. Es una señal para preguntar, no para sancionar.\n\nPara que el aviso de tanque funcione bien, carga la **Capacidad de tanque** real en la ficha de cada vehículo. Sin eso el ERP usa un estimado por categoría.",
      },
      {
        pregunta: "¿Qué me dice la comparación del final del historial?",
        respuesta:
          "Al final de todas las recargas hay una ficha que compara **este periodo contra el anterior**: con el filtro de mes en «agosto» compara agosto contra julio, y con «todos los meses» los **últimos 30 días contra los 30 anteriores**.\n\n**Lo importante no son los totales, es el reparto de abajo.** Cuando el gasto de combustible sube hay tres causas posibles y se atienden de forma muy distinta:\n\n• **Recorrió más kilómetros** — no es una anomalía, es más trabajo. Y más trabajo es más facturación.\n\n• **La unidad rinde menos km/gal** — **esto sí es una anomalía**, y es lo único de los tres que se puede arreglar: mecánica, forma de manejar, o combustible que no está llegando al tanque.\n\n• **El combustible subió de precio** — es del mercado. Cuando el diésel pasa de S/ 24.70 a S/ 25.74, el gasto sube un 4 % sin que nadie haya hecho nada mal.\n\nLa ficha reparte la diferencia entre esas tres, **cada una en soles**, y suman exacto. Por eso el renglón del rendimiento lleva la marca «lo único que se puede arreglar»: es donde mirar.\n\n**Los colores no siguen la regla de «subir es malo».** Más kilómetros sale en gris (es neutro, es trabajo); menos rendimiento y más costo por kilómetro salen en rojo.\n\n**Si tienes varias unidades**, debajo aparece la lista ordenada por cuánto se movió el rendimiento de cada una, peor primero. Sin ella el promedio esconde el problema: con diez unidades, una que empeoró 25 % mueve el total apenas un 2.5 % y no la ve nadie. Toca una placa para filtrar el historial por ella.\n\n**Cuando no se puede comparar, lo dice y explica qué falta.** Un periodo con cargas pero sin dos seguidas que tengan kilometraje no se puede medir — y ponerle el odómetro a esas filas hace aparecer la comparación.",
      },
      {
        pregunta: "¿Qué significa la columna KM del historial?",
        respuesta:
          "Son **dos números**: arriba en pequeño el **odómetro** tal como se registró en esa carga, y abajo en grande los **kilómetros recorridos desde la carga anterior** de esa misma unidad.\n\nEl de abajo es el que sirve. Un odómetro suelto (103,915) no dice nada; lo que se lee de un vistazo es «recorrió 246 km con esta carga».\n\n**Si no hay kilometraje, sale «—», nunca un cero.** Un cero se leería como «no recorrió», que es otra cosa muy distinta a «nadie lo anotó». Esas filas son además las que rompen el rendimiento de su tramo: ponerles el odómetro es la corrección que más rinde en esta pantalla.",
      },
      {
        pregunta: "Una carga que registró el Radar IA, ¿cómo la corrijo si leyó mal un número?",
        respuesta:
          "Igual que cualquier otra —✏️ en su fila— con una diferencia: **arriba del formulario aparece la foto del voucher que la IA leyó**, con el número de la nota de despacho. Ábrela tocándola: se ve en tamaño real en otra pestaña, que es lo único que sirve para leer un dígito de matriz de puntos.\n\nLas cargas que vinieron del Radar se reconocen por el **📷** al lado de la fecha, y la foto también se ve al desplegar la fila sin entrar a editar.\n\n**Corrige aquí lo que ya está en el libro.** Esta pantalla es la fuente del gasto: lo que guardes acá es lo que pesa en el costo por kilómetro, en el margen del servicio y en el precio promedio del grifo.\n\nSi la carga todavía **no** está registrada —la IA la dejó en revisión— no la corrijas aquí: atiéndela en **Radar IA → Combustible**, donde además tu corrección le queda de ejemplo a la IA para la próxima nota. Las cargas viejas pueden no tener foto guardada: en ese caso el formulario lo dice y te manda a su lectura en el Radar.",
      },
      {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    paraQueSirve: [
      ...
      "Comparar el rendimiento que tienes tecleado con el que de verdad mide tu flota, y adoptarlo si te convence.",
    ...
    faqs: [
      ...
      {
        pregunta: "En Flota / Vehículos veo una columna “Medido”. ¿Qué es y por qué no se aplica sola?",
        respuesta:
          "Es lo que **rinden de verdad** las unidades propias de esa categoría, calculado sobre las cargas de **/combustible** con el mismo motor que pinta esa pantalla: km recorridos entre dos cargas, dividido entre los galones de la segunda, y la **mediana** de todos esos tramos. Sale con la categoría **⛽ Combustible** puesta; con **🔧 Mantenimiento** la columna enseña otra medición distinta (mira la pregunta siguiente) y con **📉 Depreciación** enseña la edad de las unidades.\n\nEl número que se costea **sigue siendo el tecleado** hasta que tú apliques el otro, y eso es deliberado: bajar un rendimiento de 19 a 28.5 km/gal abarata el costo por kilómetro de esa categoría cerca de un 20 %, y como el margen se calcula sobre el precio, **abarata el precio que ofertas en toda cotización nueva de ese tipo**. Un costeo inflado se discute antes de vender; uno corto se descubre cuando el servicio ya se prestó.\n\nPor eso el chip no lleva botón: lo abre y ahí ves quién lo mide, con cuántos tramos, qué se descartó y cuántos soles mueve. El botón está dentro.\n\nEl chip te dice en qué caso estás:\n\n• **un número en verde** — hay propuesta, y el porcentaje es cuánto se aparta de lo tecleado.\n• **revisa las cargas** — hay tramos que rindieron **de más**. Eso no es buena noticia: significa que hay combustible comprado y consumido que no está registrado, así que la medición sale inflada. Se arregla registrando esa carga en /combustible.\n• **lo miden varias unidades** — dos o más placas y no dan lo mismo. Se enseñan las dos y el número lo pones tú en la celda.\n• **pocos tramos** / **sin tramos medidos** — hacen falta más cargas **con kilometraje**. Dos cargas seguidas con odómetro son un tramo.\n• **solo unidades de tercero** — no cuentan: a un proveedor se le paga por factura, no por kilómetro.\n• **usa dos combustibles** — en una unidad bimodal el rendimiento tecleado solo describe el primero, y una medición no dice cuántos km hizo con cada uno. Ahí no hay propuesta posible.\n\nY si miras la propuesta y prefieres tu número, pulsa **“No, este número lo pongo yo”**: no cambia nada, solo deja constancia de que la revisaste para que deje de proponerse hasta que la medición se mueva.",
      },
      {
        pregunta: "En 🔧 Mantenimiento también hay una columna “Medido”. ¿De dónde sale ese S/km?",
        respuesta:
          "De tus propias **órdenes de trabajo**: los soles de las OT registradas en **/mantenimiento** divididos entre los kilómetros recorridos entre ellas. Un tramo es el trecho entre dos órdenes **con kilometraje anotado**, y el costo que se le carga es el de la orden que lo **cierra** — el mantenimiento se paga después del desgaste que lo causó.\n\nTres cosas que conviene saber antes de aplicarlo:\n\n• **No es una mediana, es la suma dividida entre la suma.** En combustible se usa la mediana porque el dato raro suele ser un hueco de registro. Aquí no: el mantenimiento va a tirones, y una reparación mayor es un costo **real** que ocurre cada tantos miles de km. Si se usara la mediana, esa reparación quedaría fuera y estarías costeando solo los meses tranquilos.\n\n• **Es un piso, no un techo.** Solo cuenta lo que está asentado como orden de trabajo. Un repuesto pagado por caja chica que nunca se registró en /mantenimiento no está en ese número — por eso el modal te muestra cuántas órdenes quedaron fuera y por qué.\n\n• **Si aparece “revisa las llantas”, no apliques sin mirar.** Tu ficha ya cobra los neumáticos en su propio renglón, así que una compra de llantas registrada como orden de trabajo se estaría contando **dos veces**. El ERP no puede saber si esa orden fue una compra o una revisión que las menciona, así que no la descuenta solo: te muestra las dos cifras (con y sin) y el número lo tecleas tú.\n\nY ojo con la dirección: aquí un número medido normalmente **sube** el costo, al revés que en combustible. Eso no es malo — es el costo que ya estabas pagando y no estabas cobrando.\n\n**Cuando la columna no da un número medido, dice exactamente por qué**, porque cada caso se arregla en otro lado: *sin órdenes registradas* (nadie asentó el mantenimiento en /mantenimiento), *órdenes sin kilometraje* (hay OT pero a ninguna le anotaron el odómetro — se completa en /mantenimiento → Historial), *falta la segunda orden* (hay UNA orden con kilometraje, y un tramo son dos odómetros: **no falta ningún dato**, el trecho lo cierra el próximo servicio), *kilometrajes que no encadenan* (hay dos o más lecturas y alguna retrocede o salta lo imposible: aquí no falta un dato, hay un número equivocado) y *pocos tramos* (ya se mide, pero todavía no alcanza para un patrón — el número se enseña igual, para comparar).\n\n**Y hay un tercer número: “estimado por el plan” 📋.** Si la unidad está enrolada a un plan del fabricante que declara servicio cada X km (en /mantenimiento → Planes), el ERP proyecta el costo de tus órdenes **preventivas** sobre ese intervalo y publica el S/km del **servicio programado**, aunque todavía no haya dos odómetros que encadenen. Sirve para tener una referencia desde la primera orden — pero **no se aplica nunca, y no es por prudencia: mide otra cosa.** El parámetro es *todo* el mantenimiento por kilómetro; el estimado deja fuera los correctivos y el mantenimiento mayor por construcción, así que siempre queda por debajo. La distancia entre las dos cifras es, justamente, lo que el parámetro está absorbiendo de lo no programado.",
      },
      {
        pregunta: "Tengo buses nuevos y buses comprados usados. ¿Cómo los configuro para cobrar distinto?",
        respuesta:
          "Con **dos categorías de costeo**, no con un descuento. Todo el costeo trabaja por categoría, y cada placa apunta a una desde **/vehiculos → Editar → Categoría de costeo**, así que duplicar la ficha (una **Full Equipo (menos de 10 años)** y una **Estándar (más de 10 años)**) arregla a la vez el cotizador, el tarifario y el presupuesto de cada servicio.\n\nEn 📉 Depreciación, la columna **Flota** te avisa cuándo hace falta: cruza el **año de fabricación** de tus unidades con la vida útil que declara la ficha. Si ves **“mezcla flota nueva y usada”**, esa categoría está depreciando un bus de 2009 y uno de 2022 con el mismo valor de compra — una cifra que está mal para los dos a la vez.\n\n**Lo que no se puede hacer es bajarle solo la depreciación a la usada.** Es el error caro y la cuenta lo demuestra: una unidad comprada al 25 % ahorra alrededor de S/ 0.43 por kilómetro de capital… y un mantenimiento más caro se lo come entero. En la ficha de la estándar cambian **cuatro** números, no uno:\n\n• **Valor de compra** — lo que pagaste de verdad, no el precio 0 km.\n• **Vida útil** — los años que esperas **servir** con esa unidad. Ojo: no es el tope tributario de 5 años; si un bus comprado a los 12 va a trabajar 10 más, repartir su capital en 5 encarece cada kilómetro de más.\n• **Valor residual** — un % de lo que **pagaste**, no del valor de lista.\n• **Mantenimiento S/km** — sube, y ese es justo el que la columna Medido de 🔧 Mantenimiento te va a corregir con tus órdenes de trabajo reales.\n\nY una cosa que no cambia: los **km anuales**. La unidad usada corre la misma ruta; bajárselos le subiría artificialmente la depreciación y los gastos fijos por kilómetro.\n\nPor último: lo que decide el precio de venta no es la ficha, es el **margen**. Los tres cuadros del cotizador se llaman **Margen mínimo 15 % · Margen objetivo 20 % · Margen alto 25 %** justamente por eso: antes decían “Estándar” y “Premium”, los mismos nombres que llevaban las fichas, y con un bus de ese nivel elegido la pantalla parecía ofrecer además un “precio estándar”. Son dos ejes: **la unidad decide el costo, el margen decide el precio.** Que el servicio Full Equipo se venda con más margen es una decisión comercial; lo que la ficha tiene que garantizar es que la estándar **no se venda por debajo de su costo**.\n\nY una advertencia sobre el número que hoy está puesto: **el mantenimiento de las fichas Estándar es provisional — está igualado al de su gemela Full Equipo hasta que haya datos reales.** Eso hace que la Estándar salga más barata, pero afirma algo que nadie midió: que una unidad de más de 10 años mantiene como una de fábrica. Si gasta más taller (lo más probable), ese S/km está corto y estás vendiendo por debajo del costo real. **Lo que lo cierra es registrar las órdenes de trabajo con su kilometraje**: ahí la columna Medido te propone el número de verdad. Mientras tanto, el modal de 🔧 Mantenimiento te muestra el **punto de equilibrio** —el valor al que las dos cuestan lo mismo por km— para que veas de qué lado está cada ficha.",
      },
      {
