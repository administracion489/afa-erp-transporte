...
| Una recarga de combustible salió con fotos de **varios conductores** mezcladas | El ERP arma un reporte juntando los mensajes del mismo remitente, mismo grupo y ±10 min. Hasta el worker **1.2.0**, cuando WhatsApp no entregaba el autor se guardaba `""` en `radar_mensajes.remitente_wa` — el mismo valor para todos, así que el agrupado emparejaba a **todo el grupo**. Desde **1.3.0** un autor desconocido se guarda como `null` (y se busca en `key.participant` / `participantPn` / `participantAlt` / `msg.participant`, que con la migración a LID no siempre es el mismo campo), y el ERP además exige que el pushName no contradiga al número. Las filas ya fusionadas se recuperan con **Reprocesar** en el feed de /radar-ia. Verificar la versión en /radar-ia → chip de conexión. |
