"use client";
// ──────────────────────────────────────────────────────────────────────────────
// Pactos · Conformidades — los diferenciales esperando la firma del cliente.
//
// Cuando sube el precio de un servicio, el trigger crea el acta con un TOKEN. Hasta
// que el cliente firme, ese mayor precio es discutible: al cierre del mes se pelea
// contra alguien que ya no recuerda haber pedido el bus grande, y en la práctica se
// regala. Esta pestaña es la lista de esa plata.
//
// Todo lo que hace falta es copiar el enlace y mandarlo por WhatsApp o correo. No hay
// integración de envío acá a propósito: el operador ya está conversando con su
// contacto por el canal que sea, y obligarlo a usar otro solo agrega fricción.
// ──────────────────────────────────────────────────────────────────────────────
...
export default function ConformidadesTab({ onCambio }: { onCambio: () => void }) {
  ...
  const visibles = verTodas ? filas : pendientes;
  ...
  if (!filas.length)
    return (
      <div className="bg-white rounded-2xl border p-10 text-center">
        <p className="text-gray-500 text-sm">No hay cambios de precio esperando conformidad.</p>
        <p className="text-gray-400 text-xs mt-1">
          Se genera un enlace automáticamente cada vez que sube el precio de un servicio.
        </p>
      </div>
    );
