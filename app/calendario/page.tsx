      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="px-5 py-4" style={{ background: estCfg.border }}>
          <div className="flex items-start justify-between">
            ...
            <button onClick={onCerrar} className="text-white/70 hover:text-white text-xl font-bold">✕</button>
          </div>
        </div>
        <div className="px-5 py-4 space-y-4">
          <div className="flex gap-4">
            <div className="flex items-center gap-2 text-sm text-gray-700">
              <span>📅</span><span className="font-bold">{fmtFecha(reserva.fecha_servicio)}</span>
            </div>
            {reserva.hora_servicio && <div className="flex items-center gap-2 text-sm text-gray-600"><span>🕐</span><span>{reserva.hora_servicio.slice(0, 5)} hrs</span></div>}
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-gray-50 rounded-xl p-3">
              <p className="text-[10px] font-bold uppercase tracking-wide text-gray-400 mb-1">Vehículo</p>
          ...
          <div className="grid grid-cols-3 gap-2 text-center">
            <div className="bg-green-50 rounded-xl p-2"><p className="text-[10px] font-bold text-green-600 uppercase">Precio</p><p className="text-sm font-black text-green-700">{fmtSoles(Number(reserva.precio_cliente || 0))}</p></div>
            <div className="bg-red-50 rounded-xl p-2"><p className="text-[10px] font-bold text-red-500 uppercase">Costo</p><p className="text-sm font-black text-red-700">{fmtSoles(Number(reserva.costo_proveedor || 0))}</p></div>
          ...
          {reserva.observaciones && <div className="bg-blue-50 rounded-xl px-3 py-2 text-xs text-blue-700 italic">"{reserva.observaciones}"</div>}
          <div>
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="px-5 py-4" style={{ background: "#f59e0b" }}>
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="text-white text-xs font-bold px-2 py-0.5 rounded-full" style={{ background: "rgba(255,255,255,0.25)" }}>⚠️ Cotización aprobada sin reserva</span>
              </div>
        ...
        <div className="px-5 py-4 space-y-3">
          <div className="bg-amber-50 rounded-xl p-3 text-sm text-amber-800">
            <p className="font-bold mb-1">Esta cotización está aprobada pero aún no se convirtió en reserva.</p>
            <p className="text-xs">Ve a <b>Cotizaciones</b> y usa el botón <b>"→ Reserva"</b> para programar este servicio.</p>
          </div>
          ...
          {cotizacion.asunto && <p className="text-xs text-gray-500 italic">"{cotizacion.asunto}"</p>}
...
export default function CalendarioPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [rRes, cotRes, clRes, vRes, cRes] = await Promise.all([
        supabase.from("reservas").select("*").order("fecha_servicio").order("hora_servicio"),
      ...
      // Solo cotizaciones aprobadas que NO tienen reserva creada
      const cotData = cotRes.data || [];
      const rData = rRes.data || [];
      const cotSinReserva = cotData.filter((c: Cotizacion) =>
        !rData.some((r: Reserva) => r.cotizacion_id === c.id)
      );
      setCotizaciones(cotSinReserva);
  ...
  // Cotizaciones aprobadas sin reserva → color naranja rayado
  const eventosCotizaciones = mostrarCotAprobadas ? cotizaciones.map(c => {
    const cliente  = clientes.find(cl => cl.id === c.cliente_id);
    const nombreCl = cliente ? (cliente.empresa || cliente.nombre) : "Sin cliente";
    return {
      id:    `c-${c.id}`,
      title: `⚠️ ${nombreCl} · ${c.origen.split(",")[0]} → ${c.destino.split(",")[0]}`,
  ...
  return (
    <>
      {modalReserva && (
        <ModalReserva reserva={modalReserva}
          ...
          onCerrar={() => setModalReserva(null)} onCambiarEstado={cambiarEstado} />
      )}
      {modalCotizacion && (
        <ModalCotizacion cotizacion={modalCotizacion}
        ...
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">
              Programación visual · {resMes.length} servicios este mes
              {resHoy.length > 0 && <span className="ml-2 font-bold text-[#0b315f]">· 🚌 {resHoy.length} hoy</span>}
              {cotizaciones.length > 0 && <span className="ml-2 font-bold text-amber-600">· ⚠️ {cotizaciones.length} cotiz. aprobadas sin reserva</span>}
            </p>
          </div>
        </div>

        {/* Alerta cotizaciones aprobadas */}
        {cotizaciones.length > 0 && (
          <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 flex items-start gap-3">
            <span className="text-xl mt-0.5">⚠️</span>
            <div className="text-sm text-amber-800">
              <span className="font-bold">{cotizaciones.length} cotización{cotizaciones.length > 1 ? "es" : ""} aprobada{cotizaciones.length > 1 ? "s" : ""} sin reserva — </span>
              servicios comprometidos que no están en la agenda. Ve a <b>Cotizaciones</b> y usa "→ Reserva" para programarlos.
            </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl border shadow-sm p-4">
          <div className="flex items-center justify-between flex-wrap gap-3">
            ...
            <div className="flex gap-1 bg-gray-100 p-1 rounded-xl">
              {[{ key: "dayGridMonth", label: "Mes" }, { key: "timeGridWeek", label: "Semana" }, { key: "timeGridDay", label: "Día" }].map(v => (
            ...
            <div className="flex gap-2 flex-wrap items-center">
              ...
              <button onClick={() => setMostrarCotAprobadas(v => !v)}
                ...
                ⚠️ Cotiz. sin reserva {mostrarCotAprobadas ? "✓" : ""}
              </button>
            </div>
                ...
                <div className="px-1 py-0.5 overflow-hidden" title={arg.event.title}>
                  ...
                  {arg.view.type !== "dayGridMonth" && !esCot && r && (
                    <div className="text-white/80 truncate" style={{ fontSize: "10px" }}>
                      {v?.placa || "Sin vehículo"} · {r.hora_servicio?.slice(0, 5)}
                    </div>
                  )}
                ...
                return (
                  <div key={r.id} className="px-5 py-3 flex items-center justify-between hover:bg-gray-50 cursor-pointer transition-colors" onClick={() => setModalReserva(r)}>
                    ...
                    <div className="flex items-center gap-3">
                      {vehiculo && <span className="text-xs font-mono font-bold text-gray-600 bg-gray-100 px-2 py-0.5 rounded">{vehiculo.placa}</span>}
                      {cond && <span className="text-xs text-gray-500">{cond.nombre}</span>}
