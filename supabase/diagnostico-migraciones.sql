...
with huella(orden, modulo, script, objeto, tipo, para_que) as (values
  ...
  (2, 'Empresa · cuentas bancarias',   'empresa-02-cuentas-bancarias.sql',            'empresa_perfil.cuentas_bancarias',  'columna', 'Que el PDF de la cotización imprima TUS cuentas de abono. Sin esto no se pueden guardar y el documento sale sin ese bloque — antes iban escritas en el código.'),
  (2, 'Taller · dos o más mecánicos',  'mantenimiento-07-mecanicos.sql',              'ot_costo_linea.personal_administrativo_id', 'columna', 'Que cada hora se valorice con el costo empresa de QUIEN la trabajó. Sin esto, las horas de los dos mecánicos salen a la misma tarifa y el S/km de la categoría depende de a quién le tocó la orden.'),
  (2, 'Personal · remuneración',       'mantenimiento-07-mecanicos.sql',              'personal_administrativo.sueldo_basico', 'columna', 'El sueldo del personal de oficina, con los MISMOS nombres que en `conductores` porque alimentan la misma fórmula. Sin esto, el bloque «Remuneración» de /personal-administrativo no guarda.'),
  (2, 'Personal · vista de planilla',  'mantenimiento-07-mecanicos.sql',              'v_personal_planilla',               'vista',   'Publica sueldo y factores del régimen vigente para valorizar una hora propia. No calcula nada: la fórmula vive una sola vez en TS.'),
  (2, 'Combustible · tanque lleno',    'combustible-01-tanque-lleno.sql',             'combustible.tanque_lleno',          'columna', 'Marcar que una carga NO llenó el tanque, para que su tramo no publique un km/gal falso. Sin correrla el ERP mide exactamente como hoy; es la casilla la que no se puede guardar.'),
  (2, 'Combustible · aguja del Radar', 'combustible-02-nivel-tanque-radar.sql',       'radar_combustible.nivel_tanque',    'columna', 'Que el Radar guarde lo que vio en el indicador de nivel de la foto del tablero, para PROPONER la casilla «tanque lleno» al revisar. Sin correrla el Radar funciona igual: la propuesta simplemente no existe.'),
